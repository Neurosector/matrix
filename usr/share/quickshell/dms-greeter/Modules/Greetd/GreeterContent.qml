...
Item {
    ...
    // Der Wächter: läuft gerade eine Wiederherstellungs-Sitzung an?
    // (siehe „Passwort vergessen"-Button und onReadyToLaunch)
    property bool recoveryMode: false

    // Matrix-Schlüssel: erkennt den Login-Stick (Label MATRIXKEY) und gibt
    // sofort Feedback — Schlüssel-Symbol + Hinweis im Passwortfeld. Enter
    // genügt dann (PAM: matrix-schluessel --pam-verify, sufficient).
    property bool matrixSchluesselDa: false

    // Sicherheits-Feedback: Der Wächter-Klang macht die Umgebung darauf
    // aufmerksam, dass gerade ein Login-Schlüssel benutzt wird — bewusst
    // NICHT abschaltbar (wer den Stick unbefugt nutzt, wird gehört).
    onMatrixSchluesselDaChanged: {
        if (matrixSchluesselDa && isPrimaryScreen)
            schluesselKlang.running = true;
    }

    Process {
        id: schluesselKlang

        command: ["sh", "-c", "f=/usr/share/matrix/klaenge/13-schluessel-erkannt.wav; [ -e \"$f\" ] || f=/var/cache/dms-greeter/13-schluessel-erkannt.wav; pw-play \"$f\" 2>/dev/null || paplay \"$f\" 2>/dev/null || aplay -q \"$f\" 2>/dev/null"]
    }

    Process {
        id: schluesselCheck

        command: ["sh", "-c", "test -e /dev/disk/by-label/MATRIXKEY && echo 1 || echo 0"]
        stdout: SplitParser {
            onRead: data => root.matrixSchluesselDa = data.trim() === "1"
        }
    }
    ...
    Timer {
        ...
        onTriggered: schluesselCheck.running = true
    }
    ...
    Rectangle {
        ...
        Item {
            ...
            ColumnLayout {
                ...
                StyledText {
                    Layout.alignment: Qt.AlignHCenter
                    text: GreeterState.username ? GreeterState.username.charAt(0).toUpperCase() + GreeterState.username.slice(1) : ""
                    font.pixelSize: Theme.fontSizeXLarge
                    color: "white"
                    opacity: 0.95
                    visible: text !== ""
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        DankIcon {
                            ...
                            anchors.verticalCenter: parent.verticalCenter
                            name: GreeterState.showPasswordInput ? (root.matrixSchluesselDa ? "key" : "lock") : "person"
                            size: 20
                            color: (root.matrixSchluesselDa && GreeterState.showPasswordInput) ? Theme.primary : (inputField.activeFocus ? Theme.primary : Theme.surfaceVariantText)
                        }
                        ...
                        StyledText {
                            ...
                            text: {
                                ...
                                if (GreeterState.showPasswordInput) {
                                    return root.matrixSchluesselDa ? "Schlüssel erkannt — Enter drücken" : I18n.tr("Password...");
                                }
                                return I18n.tr("Username...");
                            }
                            color: (GreeterState.unlocking || (root.matrixSchluesselDa && GreeterState.showPasswordInput) || (Greetd.state !== GreetdState.Inactive && !awaitingExternalAuth && !pendingPasswordResponse)) ? Theme.primary : Theme.outline
                ...
                // Der Wächter: „Passwort vergessen?" → ruft die geführte
                // Wiederherstellung. Startet eine greetd-Sitzung als der
                // Recovery-Benutzer `wache` (passwortlos, nur dieser Account)
                // und ertönt dabei laut — Übernahme passiert nie heimlich.
                StyledText {
                    id: waechterRufButton
                    Layout.alignment: Qt.AlignHCenter
                    Layout.topMargin: Theme.spacingS
                    text: "Passwort vergessen?"
                    font.pixelSize: Theme.fontSizeSmall
                    color: waechterMaus.containsMouse ? Theme.primary : Theme.outline
                    opacity: (GreeterState.showPasswordInput && Greetd.state === GreetdState.Inactive) ? 1 : 0
                    visible: opacity > 0
                    ...
                    MouseArea {
                        ...
                        enabled: GreeterState.showPasswordInput && Greetd.state === GreetdState.Inactive
                        onClicked: {
                            waechterRuf.running = true;
                            root.recoveryMode = true;
                            GreeterState.reset();
                            inputField.text = "";
                            Greetd.createSession("wache");
                        }
                    }
                }
            }
        ...
        // Der Ruf des Wächters (Klang Nr. 14) — laut, damit die Umgebung
        // mitbekommt, dass jemand die Wiederherstellung öffnet.
        Process {
            id: waechterRuf
            command: ["sh", "-c", "f=/usr/share/matrix/klaenge/14-waechter-ruf.wav; [ -e \"$f\" ] || f=/var/cache/dms-greeter/14-waechter-ruf.wav; pw-play \"$f\" 2>/dev/null || paplay \"$f\" 2>/dev/null || aplay -q \"$f\" 2>/dev/null"]
        }
    ...
    Connections {
        ...
        function onReadyToLaunch() {
            ...
            passwordFailureCount = 0;
            clearAuthFeedback();
            // Wiederherstellung: statt der Nutzer-Sitzung die geführte
            // Recovery des Wächters starten (läuft als Benutzer `wache`).
            if (root.recoveryMode) {
                GreeterState.unlocking = true;
                launchTimeout.restart();
                pendingLaunchCommand = "niri -c /etc/matrix/recovery-niri.kdl";
