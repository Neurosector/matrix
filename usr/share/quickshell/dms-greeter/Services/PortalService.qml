...
Singleton {
    ...
    function getGreeterUserProfileImage(username) {
        ...
        userProfileCheckProcess.command = ["bash", "-c", `uid=$(id -u ${username} 2>/dev/null) && [ -n "$uid" ] && busctl get-property org.freedesktop.Accounts /org/freedesktop/Accounts/User$uid org.freedesktop.Accounts.User IconFile 2>/dev/null | sed -n 's/^s "\\(.*\\)"$/\\1/p' || echo ""`];
